...
class StockTransferResource extends JsonResource
{
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'transfer_type'   => $this->transfer_type ?? 'inter_company',
            'from_company_id' => $this->company_id,
            'to_company_id'   => $this->to_company_id,
            'from_location_id'=> $this->from_location_id,
            'to_location_id'  => $this->to_location_id,
            'from_company'    => $this->fromCompany?->name,
            'to_company'      => $this->isIntraCompany()
                ? ($this->toLocation?->name ? "{$this->fromCompany?->name} (location move)" : $this->fromCompany?->name)
                : $this->toCompany?->name,
            'from_location'   => $this->fromLocation?->name ?? $this->fromCompany?->name,
            'to_location'     => $this->isIntraCompany()
                ? $this->toLocation?->name
                : ($this->toCompany?->name ?? $this->toLocation?->name),
            'status'          => $this->status,
            ...
            'items'           => $this->whenLoaded('items', fn () => $this->items->map(fn ($i) => [
                ...
                'qty'              => (float) $i->qty,
                'approved_qty'     => $i->approved_qty !== null ? (float) $i->approved_qty : null,
                'rejected_qty'     => (float) ($i->rejected_qty ?? 0),
